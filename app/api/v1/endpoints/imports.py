...
@router.get("/history", response_model=list[ImportHistoryResponse], dependencies=[Depends(get_current_active_user)])
def get_history(
    page: int = 1,
    limit: int = 20,
    session: Session = Depends(get_session)
...
@router.get("/history/search", response_model=list[ImportHistoryResponse], dependencies=[Depends(get_current_active_user)])
def search_history(
    q: str = Query(...),
    session: Session = Depends(get_session)
):
